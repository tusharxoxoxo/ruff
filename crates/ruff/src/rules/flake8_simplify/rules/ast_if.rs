...
pub(crate) fn needless_bool(checker: &mut Checker, stmt: &Stmt) {
    let Stmt::If(ast::StmtIf { test: if_test, body: if_body, elif_else_clauses, range: _ }) = stmt else {
        return;
    };
    ...
    let (Some(if_return), Some(else_return)) = (is_one_line_return_bool(if_body), is_one_line_return_bool(else_body)) else {
        return;
    };
...
pub(crate) fn use_ternary_operator(checker: &mut Checker, stmt: &Stmt) {
    ...
    let [Stmt::Assign(ast::StmtAssign { targets: body_targets, value: body_value, .. })] = body.as_slice() else {
        return;
    };
    let [Stmt::Assign(ast::StmtAssign { targets: else_targets, value: else_value, .. })] = else_body.as_slice() else {
        return;
    };
